...
public class Principal {
	...
	public static void main(String[] args) {
		...
		int opcion = 0, opAprobado = 0, opQuiniela = 0, tam = 10, opParidad = 0;
		int [] quiniela;
		String paridad;
		char a;
		...
		switch (opcion ) {
			case 1:
				do {
					...
					System.out.println("Pulse 0 para volver al menú prinpipal");
			...
			case 2:
				...
				do {
					...
					if (opParidad != 0) {
						System.out.printf("El número %d es %s", opParidad, paridad.calcularParidad());
					}
				}while (opParidad != 0);
			case 4:
					...
					if (opAprobado != 0) {
						if (num1.calcularNum() == 6 && num2.calcularNum() == 6 && num3.calcularNum() == 6) {
							...
							System.out.printf("Lo sentimos. Su número es %d%d%d y por lo tanto nos vemos el "
									+ "año que viene", num1.calcularNum(), num2.calcularNum(), num3.calcularNum());
						}
					}
				}while (opAprobado != 0);
				break;
